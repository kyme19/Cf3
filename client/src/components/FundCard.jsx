...
const FundCard = ({owner, title, description, target, deadline, 
  ...
  return (
    ...
    onClick={handleClick}
    >
      <img src={image} alt="fund" className='w-full h-full rounded-[15px]' />
      <div className="flex flex-col p-4">
        ...
        <div className='flex justify-between flex-wrap mt-[15px] gap-2'>
          <div className='flex flex-col'>
            ...
            text-[#b2b3bd] leading-[22px]'>
                {amountCollected}
            </h4>
            <p className='mt-[3px] font-epilogue font-normal text-[12px]
            text-[#808191] text-left sm:max-w-[120px] truncate'>
                Raised of {target}
            </p>
          </div>
          <div className='flex flex-col'>
        ...
        <div className='flex items-center mt-[20px] gap-[12px] '>
          ...
          <p className='flex-1 font-epilogue font-normal text-[12px] text-[#808191] truncate
            '>by <span className='text-[#b2b3bs]'>{owner}</span></p>
        </div>
      </div>
    </div>
  )
